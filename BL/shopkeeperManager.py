...
def addShopkeeper(name, contact_info):
    """Validates and adds a shopkeeper to the database, then triggers backup."""
    try:
        success, message = shopkeeperDL.insertShopkeeper(name, contact_info)
        if success:
...
def updateShopkeeper(shopkeeper_id, name, contact_info):
    """Updates an existing shopkeeper in the database."""
    try:
        success, message = shopkeeperDL.updateShopkeeper(shopkeeper_id, name, contact_info)
        if success:
            backupManager.backupDatabaseTable("Shopkeepers")  # Backup only this table
